...
[![JSR Version](https://img.shields.io/jsr/v/@rotu/structview)](https://jsr.io/@rotu/structview) [![NPM Version](https://img.shields.io/npm/v/@rotu/structview)](https://www.npmjs.com/package/@rotu/structview)

# Development

Contributor workflow notes, local commands, hooks, and release steps live in `AGENTS.md`.
...
```js
import { defineArray, defineStruct, f32, string, substruct, u8 } from "@rotu/structview"
...
class Version extends defineStruct({ major: u8(0), minor: u8(1), patch: u8(2) }) {
  asString() {
...
console.log("encoded hex:", bytes.toHex())

console.log(`${info.product} v${info.version.asString()}`)

// You can compose structs into arrays
class Dish extends defineStruct({ price: f32(0), name: string(4, 12) }) {}

class Menu extends defineArray({ struct: Dish, byteStride: 16, length: 3 }) {}
...
1. Resizable structs are not yet implemented. Resizable `Arraybuffer`s only allow you to add or remove bytes at the end which is not the best experience. You can still create a `Struct` on top of a resizable `ArrayBuffer` at your own risk.
...
3. Be careful using `TypedArray`s. They have an alignment requirement relative to their underlying `ArrayBuffer`.
4. `Struct` classes define properties on the prototype, _not_ on the instance. That means spread syntax (`x = {...s}`) and `JSON.stringify(s)` will _not_ reflect inherited fields.
