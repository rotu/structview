/** @file Tools for declaring a binary struct as a class with properties */
...
/**
 * Base class for a structured binary object Note there are no predeclared string-keyed properties -
 * all property names are reserved for user-defined fields
 */
export class Struct {
   ...
   *
   * @param args Options for allocation. Note that if the class does not have a static BYTE_LENGTH
   *   property, you _must_ provide a byteLength here.
   * @returns
   */
  static alloc<T extends Struct>(
    ...
    const maxByteLength = args?.maxByteLength ?? Reflect.get(this, "MAX_BYTE_LENGTH")
  ...
  static [Symbol.hasInstance](instance: unknown): boolean {
    return typeof instance === "object" && instance !== null && dataViewSymbol in instance
  }
   ...
   *
   * @remarks
   *   The reason we don't use a positional argument for `byteLength` is because it's confusing -
   *   TypedArray constructors take a `length` argument that is in elements, not bytes. Another
   *   reason is because this allows you to pass in a `DataView` or a `TypedArray` directly to
   *   reinterpret its memory as a struct. Instead of passing an undefined `buffer` property,
   *   consider using the `.alloc` static method.
   * @param arg Options for creating the struct. If options has a `.buffer` property, we will use
   *   that as the backing memory (e.g. any TypedArray or DataView). If options has no `.buffer`
   *   property but has a `.byteLength`, we will allocate a new buffer for the object.
   */
  constructor(
    arg:
      | {
      ...
      | { readonly buffer?: undefined; readonly byteOffset?: number; readonly byteLength: number },
  ) {
...
>(ctor: Ctor, propertyDescriptors: Props): SubclassWithProperties<Ctor, MixinFromProps<Props>> {
 ...
 *
 * @param propertyDescriptors Properties to add to subclass instances. If enumerable is not
...
export function defineStruct<const Props extends PropertyDescriptorMap>(
  ...
  function defaultEnumerable(descriptor: PropertyDescriptor) {
    return Object.create(descriptor, { enumerable: { value: descriptor.enumerable ?? true } })
  }
  const newDescriptorEntries = Object.entries(propertyDescriptors).map(([name, value]) => [
    name,
    defaultEnumerable(value),
  ])
  const newDescriptors = Object.fromEntries(newDescriptorEntries) as Props
 ...
 */
export function defineArray<Item extends object>(arrayOptions: {
  ...
  /**
   * Total number of items in the array (not bytes). If omitted, the array length will depend on the
   * size of its underlying buffer
   */
  readonly length?: number
}): StructConstructor<{ readonly length: number; item(i: number): Item } & Iterable<Item>> {
  ...
  class StructArray extends Struct {
     ...
     *
     * @remarks
     *   Items are gettable but not settable. It is assumed that you will mutate the struct in
     *   place.
     * @param index
    ...
    item(index: number): Item {
      ...
      return new ctor(structBytes(this, this.#byteStride * index, this.#byteStride * (index + 1)))
    }
